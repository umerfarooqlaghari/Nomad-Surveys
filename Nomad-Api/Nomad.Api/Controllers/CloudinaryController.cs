...
[ApiController]
[Route("{tenantSlug}/api/cloudinary")]
[AuthorizeTenant]
public class CloudinaryController : ControllerBase
{
    ...
    public async Task<IActionResult> GetImage(string publicId)
    {
        try
        {
            ...
            var image = await _cloudinaryService.GetImageAsync(publicId);
            
            if (image != null)
            {
                return Ok(new { image, message = "Image retrieved successfully" });
            }
            else
            {
        ...
        {
            _logger.LogError(ex, "Error retrieving image: {PublicId}", publicId);
