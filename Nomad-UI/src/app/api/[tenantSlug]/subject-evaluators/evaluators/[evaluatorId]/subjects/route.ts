...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/${tenantSlug}/api/evaluators/${evaluatorId}/subjects`;
    ...
    return NextResponse.json(
      { error: 'Failed to fetch evaluator subjects' },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to assign subjects to evaluator' },
      { status: 500 }
    );
  }
}
