...
export async function GET(
  ...
  try {
    ...
    let backendUrl = `${BACKEND_URL}/${tenantSlug}/api/surveys/${id}/assigned-relationships`;
    ...
    console.error('Error fetching assigned relationships:', error);
    return NextResponse.json(
      { error: 'Failed to fetch assigned relationships' },
      { status: 500 }
    );
  }
}
